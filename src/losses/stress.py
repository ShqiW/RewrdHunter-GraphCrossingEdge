...
class StressLoss:
    ...
    def __init__(self, G: nx.Graph, device: torch.cuda.device, soft=True):
        """
        ...
        self.mask = torch.triu(torch.ones(self.n,
                                          self.n,
                                          dtype=torch.bool,
                                          device=device),
                               diagonal=1)
    ...
    def calculate_scale_factor(self, coords: torch.Tensor):
        ...
        d_embed_sq = squared_norms[:, None] + squared_norms[
        ...
        alpha = numerator / denominator if denominator > 0 else torch.tensor(
    ...
    def __call__(self, coords: torch.Tensor):
        ...
        d_embed_sq = squared_norms[:, None] + squared_norms[
            None, :] - 2 * coords_scaled @ coords_scaled.T
        #        d_embed = torch.sqrt(torch.clamp(d_embed_sq, min=0.0))
        d_embed = torch.sqrt(torch.clamp(d_embed_sq, min=0.0) + 1e-3)
