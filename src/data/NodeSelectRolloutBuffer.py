"""
Rollout buffer for NodeSelectGNNPolicy + RefinementGraphEnv (node_select_mode=True).

Each step: obs [N, 5], edge_index [2, 2E], edge_attr [2E, 1], action [N, 2].
Actions are variable-length (different graphs have different N), stored as a list.
"""
import copy
...
class NodeSelectRolloutBuffer(BaseRolloutBuffer):
    ...
    def _alloc(self):
        ...
        self.actions:       list = [None] * self.n_steps  # each [n_i, 2] tensor
    ...
    def get_batch_data(self, indices):
        ...
        # actions: list of [n_i, 2] — variable size, cannot stack
        actions_list = [self._actions_list[i] for i in indices]
        return sub_batch, actions_list
