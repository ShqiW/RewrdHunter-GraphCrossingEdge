...
Our best method, Sequential Refinement, achieves an SPC of $-24.58\%$
...
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=0pt]
    ...
    \item \textsc{SequentialRefinement} as the best-performing method,
          achieving SPC $-24.58\%$ vs.\ \texttt{neato} by iteratively
...
The policy (\textsc{NodeSelectGNNPolicy}) encodes a 5-dim node feature
...
It extends the GAT refinement architecture with two new components.  First, a \emph{node selection head}
...
Continuous PPO      & Continuous 2D & All nodes (simultaneous) & GAT \\
...
achieves SPC $-2.38\%$ and mean crossing count 28.65, outperforming
Discrete PPO ($-0.87\%$, mean cr 28.93) because it can express precise
displacements without a discretized direction grid.  Sequential PPO
(Method~3) is the only from-scratch method and the only one that performs
\emph{worse} than \texttt{neato} (SPC $+1.11\%$, mean cr 29.25).
Constructing a layout without any initialization is a much harder problem:
errors made early in the BFS placement sequence compound through all
subsequent nodes, and the agent must plan many steps ahead without
a reliable starting point.  Additionally, Continuous PPO exhibits severe
instability in its final-state crossings (mean 1{,}200) after early
convergence, though its best-seen count remains competitive.
...
(Method~4) achieves the best overall result: SPC $-24.58\%$ and mean
crossing count 22.45, a reduction of roughly 7~crossings per graph on
...
\begin{tabular}{lccc}
\toprule
Method & Mean cr $\downarrow$ & SPC $\downarrow$ & Zero-cr (\%) $\uparrow$ \\
\midrule
\texttt{neato} (init)             & ---   & 0.0\%     & ---  \\
Discrete PPO                      & 28.93 & $-0.87\%$ & 3.0  \\
Continuous PPO                    & 28.65 & $-2.38\%$ & 4.0  \\
Sequential PPO                    & 29.25 & $+1.11\%$ & 3.0  \\
\textbf{Seq.\ Refinement}         & \textbf{22.45} & $\mathbf{-24.58\%}$ & \textbf{4.0} \\
...
\midrule
\textbf{BFS}  & $\mathbf{-24.58\%}$ & \textbf{22.45} & \textbf{8.0} & \textbf{165} \\
...
performance gap between the from-scratch approach (Method~3, Sequential PPO)
...
Two limitations stand out from the experimental results.
...
Second, the attempt to replace fixed BFS cycling with a learned node-selection
policy (Method~5, Node-Select Refinement) largely failed: it achieves only
SPC $-5.20\%$ versus $-24.58\%$ for plain BFS cycling.  The root cause is
...
$-24.58\%$ relative to \texttt{neato} on the Rome benchmark, far exceeding
the from-scratch approach (Sequential PPO) and simpler neato-initialized methods.  The attempt to improve further with a learned
